...
agent_communication:
    ...
    - agent: "main"
      message: "CORREÇÃO BUG P0 PAGAMENTOS - Implementada correção para bug de dados vazios do pagador no MercadoPago. Adicionada função get_or_fill_payer_data() que preenche automaticamente dados do pagador (nome, email, CPF) a partir do usuário autenticado quando os dados vierem incompletos. Endpoints PIX, cartão e split atualizados para usar esta função. Backend reiniciado com sucesso."