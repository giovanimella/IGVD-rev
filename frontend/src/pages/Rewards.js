    ...
    <Layout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-2">Troque seus pontos por recompensas incríveis</p>
          <div className="mt-4 inline-flex items-center gap-3 bg-amber-50 dark:bg-amber-900/30 px-6 py-3 rounded-lg border border-amber-200 dark:border-amber-700">
            <Star className="w-6 h-6 text-amber-600 dark:text-amber-400" />
            <div>
              <p className="text-sm text-amber-700 dark:text-amber-400 font-medium">Seus Pontos</p>
              <p className="text-2xl font-outfit font-bold text-amber-900 dark:text-amber-300">{user?.points || 0}</p>
            </div>
          </div>
        </div>
                ...
                {myRedemptions.map((redemption) => (
                  <div key={redemption.id} className="p-6 flex items-center justify-between" data-testid={`redemption-item-${redemption.id}`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-semibold text-slate-900 dark:text-white">{redemption.reward?.title}</h4>
                        <p className="text-sm text-slate-600 dark:text-slate-400">Solicitado em {new Date(redemption.requested_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div>
                      {redemption.status === 'pending' && (
                        <span className="px-3 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 rounded-full text-sm font-medium flex items-center gap-2">
